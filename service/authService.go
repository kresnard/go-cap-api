...
func (s DefaultAuthService) Login(req dto.LoginRequest) (*dto.LoginResponse, *errs.AppErr) {
	...
	login, err := s.repository.FindBy(req.Username, req.Password)
	if err != nil {
		return nil, err
	}
	...
	signedToken, errss := token.SignedString([]byte("rahasia"))

	if errss != nil {
		logger.Error("failed while signing access token: " + errss.Error())
